...
public class PlayerController : MonoBehaviour
{
    ...
    public Vector2 moveDir; //移动方向（即将移动）
    //public Vector2 mobileMoveDir; //移动方向（即将移动）
    //摇杆
    ...
    void Move()
    {
        ...
        if (!MoveCheck(moveDir))
        {
            ...
            /*horizontal = horizontal == 0 ? fixedJoystick.Horizontal : horizontal;
            vertical = vertical == 0 ? fixedJoystick.Vertical : vertical;*/
    ...
    {
        //Mathf.Atan2()
        JoystickX = fixedJoystick.transform.GetChild(0).GetComponent<RectTransform>().anchoredPosition.x;
        JoystickY = fixedJoystick.transform.GetChild(0).GetComponent<RectTransform>().anchoredPosition.y;

        if (Mathf.Abs(JoystickX) > 0|| Mathf.Abs(JoystickY) > 0)
        {
            moveDir = new Vector2(JoystickX, JoystickY);
        }
        
        

        
        /*if (fixedJoystick.transform.GetChild(0).transform.position.x > 0)
        {
            moveDir = Vector2.right;
            playStepSound();
            setWalkAnim(moveDir);
        ...
        if (fixedJoystick.transform.GetChild(0).transform.position.y > 0)
        {
            moveDir = Vector2.up;
            playStepSound();
            setWalkAnim(moveDir);
